...
function PlaceSearch() {
  ...
  const tokenNext = useSelector(
    (state) => state?.GooglePlaceSearch?.GoogleApi?.next_page
  );
      ...
      <div className="container">
        <div className="row">
          <div className="col-md-12">
            {show && (
              <button
                className="Data_export_btn"
                onClick={() => downloadExcel(getdata)}
              >
            ...
            {getdata?.map((item, index) => {
              return (
                <div
                  key={index}
                  className="place-card d-flex gap-3 mb-3 border py-3  rounded shadow-sm"
                >
                  <div className="place-img"></div>
                  <div className="place-body">
                    <p className="btn  mb-3 fs-1 py-1 pointer-event-none px-2 ">
                      {item?.business_status}
                    </p>
                    <h5>{item?.name}</h5>
                    <p className="m-auto">
                      <span>
                        <TbLocationFilled color="#F7634C" size={20} />
                      </span>
                    ...
                    {item?.formatted_phone_number && (
                      <p className="m-auto mt-2">
                        <span>
                          <BsFillTelephoneFill color="#32D3E4" size={20} />
                        </span>
                        &nbsp; {item?.formatted_phone_number}
                      </p>
                    )}
                    ...
                    >
                      <FaDirections color="#4DDA4A" size={20} />
                      &nbsp; Direction
                    </a>
                  </div>
                </div>
