...
function Whatsapp() {
  ...
  const RequestFields = {
    userObjectId: getUserId()?.id,
    businessType: pageDetails?.key,
  ...
  const userReqStatus = useSelector((state)=> state?.whatsappReqStore?.data?.status)
  console.log(userReqStatus)

  return (
    <>
    {userReqStatus === false && (<img src={BannerImg} className="img-fluid rounded-top" alt=""/>) } 
          ...
          {userReqStatus === false ? (  <Form
            ...
            render={({
              ...
              <form onSubmit={handleSubmit}>
                ...
                <button type='submit' className='btn btn-dark text-white'> Send Request</button>
                    </div>
